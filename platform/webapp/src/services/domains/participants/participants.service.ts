...
const rawParticipantsService = {
   ...
   * List participants
   */
  async getParticipant(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v0/participants` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getParticipant(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v0/participants`;
  ...
  async getParticipant(participantId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    ...
    const url = `/orgs/${orgId}//v0/participants/${participantId}` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getChallenge(participantId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v0/participants/${participantId}/challenge`;
   ...
   */
  async createApproveRole(participantId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v0/participants/${participantId}/approve-role`;
