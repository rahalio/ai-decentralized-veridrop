...
export async function startTestDb(): Promise<DynamoDBClient> {
  ...
  if (!process.env.TABLE_NAME && !process.env.DYNAMODB_CORE_TABLE_NAME) {
    process.env.TABLE_NAME = "veridrop-core-test";
    process.env.DYNAMODB_CORE_TABLE_NAME = "veridrop-core-test";
  }
  if (!process.env.BASE_TABLE_NAME && !process.env.DYNAMODB_BASE_TABLE_NAME) {
    process.env.BASE_TABLE_NAME = "veridrop-base-test";
    process.env.DYNAMODB_BASE_TABLE_NAME = "veridrop-base-test";
  }
  if (!process.env.ANALYTICS_TABLE_NAME && !process.env.DYNAMODB_ANALYTICS_TABLE_NAME) {
    process.env.ANALYTICS_TABLE_NAME = "veridrop-analytics-test";
    process.env.DYNAMODB_ANALYTICS_TABLE_NAME = "veridrop-analytics-test";
  }
