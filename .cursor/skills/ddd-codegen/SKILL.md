...
Package scope: `@veridrop` (via `package_scope` in `.codegen` config). `.codegen/` is local-only — never commit.
...
2. Set `package_scope` (e.g. `@acme`) and rename `@veridrop/*` packages.
